	...
	public function process_checkout() {
		try {
			...
			if ( empty( $posted_data['woocommerce_checkout_update_totals'] ) && 0 === wc_notice_count( 'error' ) ) {
				...
				$check = $deeplosystem->checkUser($current_user->ID);								
				if ($check) {
					$moneySpent = $order->get_subtotal();
					$moneyCheck = ($moneySpent / 50.00);
					$feeTotal = WC()->cart->get_fee_total();
					$feeCheck = (abs($feeTotal / 5.00));
					echo "ciao";
					$ifIsYear = $deeplosystem->ifIsYear($current_user->ID);
					//if discount is > 5€ remove diamonds of every 5€ and If is registration < 1 year
					if (($ifIsYear == 0)) {
						if (($feeCheck > 0)) {
							$check = $deeplosystem->changeDiamonds($current_user->ID,$feeCheck,0);
							$moneyQuery = $deeplosystem->addMoneySpent($current_user->ID,10);
						}
						if (($moneyCheck) > 0) {
							$y = 0;
							for ($x = 1; $x <= $moneyCheck; $x++) {
								$y = $y + 1;
							}
							$diamondsQuery = $deeplosystem->changeDiamonds($current_user->ID,$y,1);
						}
					}
					//If moneySpent is > 50€ then add diamonds
					$moneyQuery = $deeplosystem->addMoneySpent($current_user->ID,$moneySpent);
				}else{
				}
